...
\begin{figure}[h]
	\begin{subfigure}
		...
		\caption{A binary--mixture of two partially miscible fluids under three--dimensional periodic boundary conditions is used. 
The pressure and temperature are controlled using a Nos\'{e}--Hoover barostat and thermostat.
}
		\label{AABB}
	\end{subfigure}
	\begin{subfigure}
		\includegraphics[scale=0.25]{AABB_piston.png}
		\caption{A binary--mixture confined between two planar walls with periodic boundary conditions in the $(x,y)$ plane is also studied.
A Nos\'{e}--Hoover thermostat is used and the pressure is regulated by employing the confining walls as a piston.}
		\label{AABB_piston}
	\end{subfigure}
	\caption{The systems used for simulations of the Marangoni flow both involve a partially miscible binary--mixture of Fluid A (blue) and Fluid B (green).}
	\label{SetUp}
\end{figure}
...
This introduces a fictious frictional force into the equations of motion which adjusts the particle velocities until the temperature is equal to the desired value.\cite{NoseHoover1, NoseHoover2, NoseHoover3}
...
Solid walls in the system were constructed with a harmonically bonded lattice using a spring constant of $K^{*} = 2500$ and an equilbrium length of $r^{*}_{0}=1.163$.
